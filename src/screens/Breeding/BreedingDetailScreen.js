...
const BreedingDetailScreen = ({ route, navigation }) => {
  ...
  const renderInfoTab = () => (
    <View>
      {/* Description */}
      <DetailSection title="Hakkında" icon="document-text-outline" iconColor={theme.accent} cardBg={theme.card} textColor={theme.text}>
      ...
      {/* Basic Info Grid */}
      <DetailSection title="Temel Bilgiler" icon="grid-outline" iconColor={theme.accent} cardBg={theme.card} textColor={theme.text}>
        <View className="flex-row flex-wrap gap-3">
          {[
            ...
            <View key={index} style={{ backgroundColor: theme.bg, minWidth: '45%' }} className="flex-1 p-4 rounded-xl items-center">
              <View style={{ backgroundColor: theme.accentLight }} className="w-12 h-12 rounded-full items-center justify-center mb-2">
      ...
      {/* Owner Info */}
      <DetailSection title="Sahip Bilgisi" icon="person-outline" iconColor={theme.accent} cardBg={theme.card} textColor={theme.text}>
  ...
  const renderPedigreeTab = () => (
    <View>
      {/* Pedigree Info */}
      <DetailSection title="Soy Ağacı Bilgileri" icon="ribbon-outline" iconColor={theme.accent} cardBg={theme.card} textColor={theme.text}>
        <View style={{ backgroundColor: theme.accentLight }} className="p-4 rounded-xl mb-4">
          <View className="flex-row items-center gap-2 mb-4 pb-3" style={{ borderBottomWidth: 1, borderBottomColor: theme.accent + '40' }}>
            <View style={{ backgroundColor: theme.accent }} className="w-10 h-10 rounded-full items-center justify-center">
      ...
      {petDetails.breedingHistory && petDetails.breedingHistory.length > 0 && (
        <DetailSection title="Çiftleştirme Geçmişi" icon="calendar-outline" iconColor={theme.accent} cardBg={theme.card} textColor={theme.text}>
          <View className="gap-3">
      ...
      {/* Documents */}
      <DetailSection title="Belgeler" icon="document-outline" iconColor={theme.accent} cardBg={theme.card} textColor={theme.text}>
        <View className="gap-3">
          {[
            ...
            <TouchableOpacity
              ...
              accessibilityRole="button"
            >
              <View style={{ backgroundColor: doc.color + '20' }} className="w-11 h-11 rounded-full items-center justify-center mr-3">
                <Ionicons name={doc.icon} size={22} color={doc.color} />
              </View>
              <View className="flex-1">
                <Text style={{ color: theme.text, fontSize: 15, fontWeight: '600', marginBottom: 2 }}>{doc.title}</Text>
  ...
  const renderHealthTab = () => (
    <View>
      {/* Vaccinations */}
      <DetailSection title="Aşı Bilgileri" icon="shield-checkmark-outline" iconColor={theme.accent} cardBg={theme.card} textColor={theme.text}>
        <View className="gap-3">
          {petDetails.vaccinations.map((vaccine, index) => (
            <View key={index} style={{ backgroundColor: theme.bg }} className="flex-row items-center p-3 rounded-xl">
              <View style={{ backgroundColor: '#10B98120' }} className="w-11 h-11 rounded-full items-center justify-center mr-3">
                <Ionicons name="shield-checkmark" size={22} color="#10B981" />
              </View>
              <View className="flex-1">
                <Text style={{ color: theme.text, fontSize: 15, fontWeight: '600', marginBottom: 3 }}>{vaccine.name}</Text>
      ...
      {/* Health Reports */}
      <DetailSection title="Sağlık Raporları" icon="document-text-outline" iconColor={theme.accent} cardBg={theme.card} textColor={theme.text}>
        <View className="gap-3">
          {petDetails.healthReports.map((report, index) => (
            <View key={index} style={{ backgroundColor: theme.bg }} className="flex-row items-center p-3 rounded-xl">
              <View style={{ backgroundColor: '#3B82F620' }} className="w-11 h-11 rounded-full items-center justify-center mr-3">
                <Ionicons name="document-text" size={22} color="#3B82F6" />
              </View>
              <View className="flex-1">
                <Text style={{ color: theme.text, fontSize: 15, fontWeight: '600', marginBottom: 3 }}>{report.type}</Text>
      ...
      {/* Health Summary */}
      <DetailSection title="Sağlık Özeti" icon="heart-outline" iconColor={theme.accent} cardBg={theme.card} textColor={theme.text}>
        <View style={{ backgroundColor: '#10B98120' }} className="p-4 rounded-xl">
          <View className="flex-row items-center gap-3 mb-4">
            <View style={{ backgroundColor: '#10B981' }} className="w-14 h-14 rounded-full items-center justify-center">
              <Ionicons name="heart" size={28} color="#FFF" />
            </View>
            <View className="flex-1">
              <Text style={{ color: '#10B981', fontSize: 17, fontWeight: '700', marginBottom: 2 }}>Mükemmel Sağlık</Text>
      ...
      >
        ...
        <View style={{ paddingTop: 8 }}>
          ...
          <View className="px-4 mb-4">
            ...
            <View style={{ backgroundColor: theme.card }} className="flex-row items-center justify-around py-4 rounded-2xl">
              {[
